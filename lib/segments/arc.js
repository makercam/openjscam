...
class Arc {
    constructor(...args) {
        if (args.length === 4) {
            ...
            this.radius = args[1];
            this.angle = args[2];
            this.plane = args[3] || constants_1.XZ;
        }
        else {
            this.offset = args[0];
            this.angle = args[1];
            this.plane = args[2] || constants_1.XZ;
        }
    }
    getCurveForInCoord(inCoord) {
        ...
        return new THREE.EllipseCurve(centerCoord.x, centerCoord.y, this.radius || radius, this.radius || radius, radiansStart, radiansEnd, this.angle > 0, angleRadians);
    }
